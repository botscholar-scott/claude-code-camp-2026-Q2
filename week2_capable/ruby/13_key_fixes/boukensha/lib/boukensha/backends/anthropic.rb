...
module Boukensha
  module Backends
    class Anthropic < Base
      ...
      # model).
      # Anthropic returns six stop reasons. Collapsing all five non-tool_use
      # values to "end_turn" reported a reply cut off at the output ceiling as
      # a completed turn, and dropped any tool_use blocks it carried.
      def parse_response(response)
        raw     = response["stop_reason"]
        content = (response["content"] || []).map { |block| normalize_block(block) }
        { stop_reason: normalize_stop_reason(raw), raw_stop_reason: raw, content: content }
      end
