  ...
  class Agent
    ...
    def run
      ...
      loop do
        ...
        if parsed[:stop_reason] == "tool_use"
          ...
          # The transcript gets the provider's own word, not our normalization,
          # so "refusal" and "pause_turn" stay legible after the fact.
          @logger.response(text: text, usage: response["usage"],
                           stop_reason: parsed[:raw_stop_reason] || parsed[:stop_reason],
                           task: nil, backend: @builder.backend)

          # Truncation is not completion: the model was cut off at the output
          # ceiling mid-sentence, and any tool calls it had begun are in that
          # reply unfinished and undispatched. Saying "completed" here is how a
          # decomposition layer marks a subgoal done that never ran.
          @logger.turn_end(reason: truncated ? "max_output_tokens" : "completed",
                           iterations: @iteration, tokens: @context.turn_tokens)
    ...
    def handle_tool_calls(content, response)
      ...
      tool_calls.each do |block|
        ...
        @logger.tool_call(name: name, args: args, tool_use_id: use_id)
        begin
          result = @registry.dispatch(name, args)
          @logger.tool_result(name: name, result: result, tool_use_id: use_id,
                              ok: !result.error?, error: (result.text if result.error?))
        # Only the model's own mistakes are handed back for it to recover from.
        # Anything else — a crashed MCP subprocess, a broken pipe — is
        # infrastructure, and returning it as an ordinary tool result is how
        # the agent ends up issuing two dozen more commands into nothing and
        # then summarising a session that never happened. Let it propagate.
        rescue UnknownToolError, ArgumentError => e
          result = ToolResult.new("ERROR: #{e.class}: #{e.message}", true)
          @logger.tool_result(name: name, result: result, tool_use_id: use_id,
