...
**Moved to `docs/plans/ruby/14_improved_navigation.md`.** The cost analysis,
the two-graph design, what tbaMUD actually emits, how the map gets recorded and
the build order all live there now, with corrections to two claims that turned
out to be wrong (F34 is not the blocked-move sensor, and the replay corpus is 2
usable files with 99 moves, not 11 with 129).

Only §12.10 stays here, because it is about this document's own F-numbers.

### 12.10 What this does to the fix list

| defect | was | becomes | why |
|---|---|---|---|
| **F34** | §1.1 broken | **landed (§17)**, but *not* the map's sensor | it was argued to be the thing that tells a failed move from a successful one. It is not: mud-manager sets `isError` only for `ProtocolError`, so a closed door is `isError: false`. What it buys the map is knowing when a call was not an observation at all. See `14_improved_navigation.md` §7.1 |
| **F4** | §1.1 broken | **landed (§17)** | same door, infrastructure side. Its headline case, a peer that goes quiet mid-session, turned out to be neither F4 nor F34 but a swallowed timeout in mud-manager. See §17.3 |
