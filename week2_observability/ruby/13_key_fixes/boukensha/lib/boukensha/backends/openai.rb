  ...
  module Backends
    ...
    class OpenAI < Base
      ...
      def parse_response(response)
        ...
        # The Responses API reports truncation as status "incomplete" with the
        # cause in incomplete_details.reason, not as a stop reason.
        raw = response.dig("incomplete_details", "reason") || response["status"]
        { stop_reason:     function_calls.empty? ? normalize_stop_reason(raw) : "tool_use",
          raw_stop_reason: raw,
          content:         content }
      end
