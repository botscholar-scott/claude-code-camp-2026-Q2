...
class Backend(ABC):
    ...
    def parse_response(self, response: Mapping[str, Any]) -> Reply:
        ...
        The inbound direction, and the inverse of `to_messages`. It is what lets
        step 05's `Agent` carry a single `if reply.stop_reason is TOOL_USE`
        branch instead of one per provider — the loop never inspects a raw
        response.

        Ruby's five backends each implement this plus a private
        `assistant_message` that rebuilds a provider-specific assistant turn from
        the normalized blocks. Anthropic's `content` array doubles as both the
        normalized shape and the wire format, so ours needs no such inverse.
        """
