...
class AnthropicBackend(Backend):
    ...
    **2026-07-25**. It is not fetched, and it does not expire on its own — see
    ...
    def to_messages(self, context: Context) -> list[dict[str, Any]]:
        ...
        An assistant message whose content is a block sequence — the one step
        05's loop stores so a `tool_use` precedes its `tool_result` — serializes
        back to a content list. A `str` still passes straight through, so every
        payload steps 03 and 04 produced is byte-identical.
        """
        ...
        for message in context.messages:
            if message.role is Role.TOOL_RESULT:
                ...
                # Added only when set, so unaffected payloads stay identical.
                if message.is_error:
                    block["is_error"] = True
                ...
                content = (
                    ...
                    else [self._to_wire(b) for b in message.content]
                )
                messages.append({"role": message.role.value, "content": content})
        return messages

    @staticmethod
    def _to_wire(block: ContentBlock) -> dict[str, Any]:
        """One normalized block back in Anthropic's own shape.

        Nearly the identity function — Anthropic's `content` array *is* the
        normalized shape — which is why only this backend gets away without the
        `assistant_message` rebuild Ruby's other four need.
        """
    ...
    def parse_response(self, response: Mapping[str, Any]) -> Reply:
        """One Messages API response as a `Reply`.

        Two deliberate behaviours:

        **Unknown block types are dropped.** Anthropic can return `thinking`
        blocks; nothing here enables thinking, and dropping them keeps
        `ContentBlock` a closed union. Turning thinking on means extending the
        union *and* preserving the blocks on replay — see the README.

        **An unrecognised `stop_reason` becomes `OTHER`, not an exception.**
        `StopReason._missing_` absorbs `refusal`, `pause_turn`, `stop_sequence`
        and anything Anthropic adds next; `raw_stop_reason` keeps the wire value
        verbatim. A missing field lands there too — `StopReason("")` is `OTHER`,
        which ends the run rather than looping on a response we cannot read.
        """
        raw_stop_reason = response.get("stop_reason") or ""

        blocks: list[ContentBlock] = []
        for raw in response.get("content") or ():
            if not isinstance(raw, Mapping):
                continue
            match raw.get("type"):
                case "text":
                    blocks.append(TextBlock(raw.get("text") or ""))
                case "tool_use":
                    blocks.append(
                        ToolUseBlock(
                            id=raw.get("id") or "",
