...
class BoukenshaError(Exception):
    ...
    will be wanted: step 03 adds an unsupported-model error and step 04 adds
    `ApiError`. Retrofitting a base once those are in use would change their MRO.

    **Step 05 adds none.** Ruby's `errors.rb` declares `LoopError` here and in
    six later steps, and `repl.rb` rescues it in five of them (08–12) — but
    `grep -rn 'raise LoopError'` across all twelve Ruby steps finds nothing. The
    design intended the iteration ceiling to raise; the wind-down call was built
    instead and the vestige was never removed. A class that cannot be raised is
    not ported.
    """
