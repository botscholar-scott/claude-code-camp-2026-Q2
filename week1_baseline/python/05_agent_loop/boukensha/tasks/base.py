...
#: Model round-trips the agent loop may run before winding down, when settings do
#: not name a number. `0` disables the ceiling.
#:
#: Ruby declares 25 twice — `Tasks::Base::DEFAULT_MAX_ITERATIONS` *and*
#: `Agent::MAX_ITERATIONS` — which is the same defect the 1024 `max_tokens`
#: default had at step 03. It lives here, where the setting is read, and
#: `agent.py` imports it.
