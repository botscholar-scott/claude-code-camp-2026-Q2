...
class Context:
    ...
    state of a run, and messages arrive on every iteration. `Message` is the
    frozen value object it holds. See
    `docs/adr/0003-context-is-the-mutable-object.md`.

    Step 05's `Agent` **borrows** a context; it does not own or replace one, so a
    caller may hydrate a context from disk, run, and write it back.
    ...
    def messages(self) -> Sequence[Message]:
        """The full conversation history, replayed on every iteration.

        Read-only by type, not by enforcement — Python has no zero-copy
        read-only list view, and a defensive copy would be paid once per
        iteration by the agent loop. `Registry.tools` *is* enforced; the
    ...
    def add_message(
        ...
        *,
        ...
        `content` takes a block sequence for the assistant message step 05's loop
        replays; `is_error` marks a `TOOL_RESULT` the model should read as a
        failure. Neither needs a new method — one tool result per message is what
        the loop appends, so there is no `add_tool_results` batch form.
        """
        self._messages.append(Message(role, content, tool_use_id, is_error))
